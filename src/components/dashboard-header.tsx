...
export function DashboardHeader() {
  ...
  const displayName = auth.currentUser?.displayName || currentUser?.name || "Rocky";
    ...
    <header className="sticky top-0 z-50 w-full gradient-hero text-primary-foreground">
      <div className="relative mx-auto flex max-w-[800px] items-center justify-between px-6 pt-5 pb-5">
        <div className="relative z-10 flex items-center gap-3">
          <Link
            aria-label="Profile"
            className="tap grid size-11 place-items-center overflow-hidden rounded-2xl bg-primary-foreground/15 text-xl backdrop-blur-sm shadow-sm"
            to="/profile"
          >
            {showUserPhoto ? (
              <img
